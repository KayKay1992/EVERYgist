       ...
       {dashboardData && (
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md shadow-gray-100 border border-gray-200/50 mt-6">
          <div className="">
            <div className="col-span-3">
              <h2 className="text-2xl md:text-2xl font-medium">Good morning! {user?.name}</h2>
          ...
          <div className=" grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-6 mt-5">
           <DashboardSummaryCard label="Total Posts"  bgColor="bg-sky-100/60" value={dashboardData?.stats?.totalPosts || 0} color="text-sky-500" icon={<LuGalleryVerticalEnd />}/>

            <DashboardSummaryCard label="Published" bgColor="bg-green-100/60" value={dashboardData?.stats?.published || 0} color="text-orange-500" icon={<LuCheckCheck />}/>

            <DashboardSummaryCard label="Total Views" bgColor="bg-blue-100/60" value={dashboardData?.stats?.totalViews || 0} color="text-blue-500" icon={<LuChartLine />}/>

            <DashboardSummaryCard label="Total Likes" bgColor="bg-yellow-100/60" value={dashboardData?.stats?.totalLikes || 0} color="text-yellow-500" icon={<LuHeart />}/>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 my-4 md:my-6">
          ...
          <div className="col-span-12 bg-white p-6 rounded-2xl shadow-md shadow-gray-100 border border-gray-200/50 ">
            <div className="flex justify-between items-center">
              <h5 className="text-lg font-medium mt-8 mb-4">Recent Comments</h5>
            </div>
          </div>
        </div>
        </>
